...
- Name: gui-main-path.spec.js >> 4) 业务失败：model-FAIL → 唯一终态 failed（绝不随后 completed）
- Location: test\e2e\gui-main-path.spec.js:199:1
...
Call log:
  ...
  - waiting for locator('#status-text')
    63 × locator resolved to <span id="status-text">就绪</span>
       - unexpected value "就绪"

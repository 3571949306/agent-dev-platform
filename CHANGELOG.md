...
- B24 Command Palette: Ctrl/Cmd+Shift+P opens a searchable command palette (fuzzy filter, arrow-key navigation, Enter to execute, Esc to close, click-outside to dismiss). Commands are navigation / existing-user-action only (open pages, toggle panels/sidebar, new chat, stop run) — the palette never bypasses Permission and introduces no second IPC path; every command reuses an existing DOM action (`pages.open` / `panels.activate` / button click).
- B25 Keyboard shortcuts: Ctrl+Shift+P palette · Ctrl+N new chat (suppressed while typing to protect drafts) · Ctrl+J toggle bottom panel · Ctrl+B toggle sidebar · Ctrl+Shift+E left Files · Ctrl+Shift+R Problems. Enter/Shift+Enter composer behavior unchanged. Shortcuts chosen to avoid Electron default-menu accelerators.
- E2E proof: new `gui-main-path` case 10 switches theme→light + density→compact in a real Electron window, asserts `html[data-theme]`/`html[data-density]` update and `ui.theme` persists, then restores defaults; case 11 opens the palette via Ctrl+Shift+P, filters to 设置, executes it (settings page opens, palette closes), and closes via Esc; full E2E now 67/67. No existing DOM contract broken.

Remaining Phase B (B1 workbench shell/activity bar, B4 file explorer, B5–B9 task workspace/action cards/diff/run center/run tree, B10–B23 permission/workflow/generator/agents/connections/computer/diagnostics/error-center, B26–B30 quick-open/composer/onboarding/boot, B34–B48 responsive/perf/subscription/status-vocabulary/i18n, B49–B61 expanded E2E + GUI smoke/soak + production scenarios, B62–B72 docs/gates) is NOT_STARTED and will not be claimed until each has observable proof.

## v2.9.8 — Real Project Reliability

### Final Reliability Closure (Phase A)

- A1 Terminal Truth all exit paths: every terminal path (normal / catch / timeout / user-cancel / late provider) now reads the single terminal truth from `RunManager.getRun(runId).status`; the catch path no longer re-infers the final fact. Timeout aborts carry an explicit `{ type: 'timeout' }` reason so the loop settles `timeout` (not `cancelled`). A timeout racing a late model completion yields exactly one terminal event (`RUN_TIMEOUT=1`, `RUN_CANCELLED=0`, `RUN_COMPLETED=0`) and the late completion never revives the run.
- A2 Execution-Started Truth: `runId != null` is no longer accepted as proof a child actually executed. Delegate results carry an explicit `executionStarted` flag set true only after `adapter.startTask` really runs; pre-start failures (PROJECT_LOCKED / definition-not-found / lock contention) keep `executionStarted=false` and never trigger repair, while an actually-started child reaching `failed`/`timeout`/`cancelled`/`interrupted` does.
- A3 Authentic Run Tree Lineage: RunManager is now the lineage authority with `getParentRunId` / `getRootRunId` / `belongsToRoot`, deriving root by walking the persisted `parentRunId` chain (with cycle protection). Unknown runIds are fail-closed (`null` / `false`).
- A4 Persisted Lineage: child runs record real `parentRunId`/`rootRunId`/`depth` in RunManager (not only Lifecycle metadata / task objects / orchestrator memory), so RunManager can answer the true lineage. The hub-side public-identity run deliberately does not duplicate lineage to avoid double child nodes in the run tree.
- A5 Unforgeable Project Lock Reentrancy: lock reentrancy can no longer be claimed via a self-reported `task.rootRunId`. Both the child's and the lock holder's roots are derived from RunManager lineage, then cross-checked against a per-orchestrator unforgeable `delegationToken` verified by a `delegationAuthorityVerifier` injected in production. A forged `parentRunId`/token (no real active orchestrator) is rejected with `PROJECT_LOCKED`; an underivable parent lineage is fail-closed.
- A6 Real Three-Level Delegation: production proof actually runs ProductEntry → Main → Dynamic Child → Dynamic Grandchild, with the grandchild genuinely invoking `adapter.startTask` + a provider call and completing (never `PROJECT_LOCKED`). Identities verified: Main `parentRunId=null/rootRunId=Main`; Child `parent=Main/root=Main`; Grandchild `parent=Child/root=Main`.
- A7 Forged Root Attack: an independent same-project Run forging `rootRunId=<Main A>` fails busy with honest `PROJECT_LOCKED` and performs zero filesystem mutation.
- A8 Descendant Quiescence: distinguishes "child terminal flag" from "child execution quiesced". The root project lock is released only after owned descendants' runtime execution + AgentHub lifecycle + dynamic instances + owned terminal processes all truly stop (bounded 5s probe, then forced progress — never an indefinite lock hold).
- A9 Same-project Cancellation Race: cancelling a root while a grandchild terminal process is alive keeps the lock held (Run B `PROJECT_LOCKED`, 0 mutation); after the tree quiesces the process tree is killed, the lock releases, and Run B may then start.
- A10 Provider Timeout: retains internal AbortController + caller-cancellation forwarding + adapter-timeout abort + late-result guard; re-verified `providerAbortObserved=YES`, `lateSettleIgnored=YES`, `callerSignalNotMutated=YES`.

Phase A gate: full `npm test` 1672 pass / 0 fail / 1 skip; Architecture + Architecture-Policy PASS (DEFAULT_DENY, 0 unsafe duplicates, synthetic unknown paths ALL BLOCKED); Reliability suite 49/49; Reliability-Production smoke 32 machine proof tokens PASS (incl. A1–A9); Reliability Soak 20/20 fresh repos; E2E 65/65. Paid provider calls 0. Version remains 2.9.8.

Note (E2E agent-hub #32): the capability-routing assertion for Codex is now conditioned on Codex actually being installed (matching test 31's environment-aware contract). When Codex is absent, scoring it `UNAVAILABLE` is the correct behavior and is asserted as such; the Native>WorkBuddy assertion remains unconditional. This is an environment-robustness fix, not a weakening of routing coverage.

### Base Reliability (R0–R8)

- R0 Flake closure: classified the provider-abort timing flake as REAL_FLAKE and fixed the test contract at root cause (unhandled-rejection guard, bounded fixture retry for pre-abort transient loopback failures, abort-to-reject latency contract); the abort contract itself was not weakened (20/20 deterministic).
- R1 Dirty worktree preservation: user uncommitted, staged, and untracked state survives real agent runs byte-for-byte; automatic checkpoints never mutate the worktree. Destructive Git commands (reset/clean/checkout/restore/stash/switch -f) now route to the `terminal.dangerous` permission scope via CommandRiskAnalyzer; denied permission means zero process spawn.
- R2 Checkpoint truthfulness: checkpoint create is non-mutating (temporary index + write-tree + commit-tree + `refs/adp-checkpoints/<id>`), restore is exact by checkpoint ID with an automatic emergency checkpoint, and non-Git projects report `CHECKPOINT_UNSUPPORTED` instead of fake success. `git stash push` is no longer used for checkpoints.
- R3 Atomic file mutation: observation-ledger stale-write protection (`FILE_CHANGED_SINCE_READ`; `expected_sha256` must be real read evidence), same-directory temp + atomic replace writes with fault-injection proof, exclusive `create_file`, and move/copy destination collision failures unless explicitly replaced.
- R4 Verification freshness: mutation-sequence verdict rejects stale test PASS and fresh test FAIL at completion; `verificationStatus` truth (`PASS`/`FAIL`/`NOT_AVAILABLE`) is carried on the run-completed event. Fixed a latent blackboard pollution bug that turned recoverable runs into false `AGENT_REPAIR_LIMIT` failures.
- R5 Bounded transient retry: providers retry at most once (2 attempts per decision) and only for connection reset / HTTP 408/429/5xx; cancelled, timed-out, and non-transient requests are never retried.
- R6 Hang / Cancel / Cleanup: model hang settles honest `timeout` via the configured adapter timeout even when the provider never settles (new bounded-settle guard in ProviderModelAdapter); cancel during model/tool/delegate/verification settles `cancelled` in bounded time, kills the whole process tree (no late side effects), rejects late terminal revival, and zeroes every resource counter (activeRuns / dynamic instances / AgentHub active / project locks / pending approvals / owned child processes / retry timers). Dynamic-child hang never fakes child success: a hung child settles terminal via its budget, the parent fails honestly through the repair gate instead of completing (delegate timeout/failed/cancelled now routes to repair; guard/policy blocks stay observable tool feedback). Restart truth: persisted active Run/Workflow/Generator records settle `interrupted`/`FAILED(WORKFLOW_INTERRUPTED)`/`FAILED(GENERATOR_INTERRUPTED)` on cold start with zero provider/tool/mutation/step replay and no automatic resume. Terminal Audit + bounded long-task execution from the earlier round are retained as supplemental proof.
- R7 Project Lock / Run Isolation: Main Agent runs now acquire the shared ProjectMutationLock before starting (fail busy with honest `PROJECT_LOCKED`, zero mutations) and release it on every terminal path (completed/failed/cancelled/timeout). Delegated children execute reentrantly under their parent's lock; unrelated runs on the same canonical projectRoot are strictly serialized while different projects never contend. Lock holder identity is the real runId/agentId/canonical projectRoot — never conversationId. Production proof runs through the full ProductEntry → Main Agent → ActionExecutor → Tool chain.
- R8 Real Project Production Matrix + Soak: new `test:reliability` / `test:reliability:production` / `test:reliability:soak` gates. The production matrix drives the real ProductEntry chain over real dirty Git projects with a real `node --test` fail → repair → pass cycle (user markers preserved, HEAD untouched, no stash), stale-verification blocking, concurrent-external-edit protection, cancellation with real child processes, and the R7 lock scenarios; the soak runs 20/20 fresh repos end-to-end with per-iteration resource proof.
- Architecture Freeze preserved: DEFAULT_DENY execution path policy unchanged, zero unsafe duplicates, no new framework/runtime/router/engine; all reliability work strengthens existing canonical modules.
- Final verification: 1661 pass / 0 fail / 1 skip (1662 unit tests, full `npm test` repeated 3/3), all focused and production suites pass, Architecture/Product gates pass (DEFAULT_DENY, 0 unsafe duplicates), Reliability Production matrix repeated 10/10 with 21 machine proof tokens per round, provider-abort critical 20/20, Reliability Soak 20/20 fresh repos, E2E 65/65, Windows NSIS + portable builds pass, paid provider calls zero.

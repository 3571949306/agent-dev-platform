...
export async function render() {
  const box = $('#left-runs');
  box.innerHTML = `<div class="runs-toolbar"><input id="run-search" placeholder="Search goal, agent, runId" value="${esc(search)}"><div class="run-filters">${['all', 'running', 'waiting', 'completed', 'failed', 'cancelled', 'timeout', 'interrupted'].map(value => `<button class="run-filter ${filter === value ? 'active' : ''}" data-run-filter="${value}">${value}</button>`).join('')}</div></div><div id="run-list"><div class="empty small" data-page-state="empty">Loading…</div></div>`;

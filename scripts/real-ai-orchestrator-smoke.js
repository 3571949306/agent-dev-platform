'use strict';
/**
 * Real AI Orchestrator Smoke — v2.9.0 Real Runtime Smoke Closure（R1-R9）。
 *
 ...
 *
 * 与旧版的关键区别：
 *   - getTool：生产 src/tools/registry.js Built-in Tools（禁止 null / fake）。
 *   - PathSecurity：生产 src/security/pathSecurity（TEMP fixture 为 projectRoot），
 *     并自动做 deterministic outside-write 拒绝断言。
 *   - PermissionEngine：生产实例，仅授权项目内 fs/terminal + subagent，其余显式 deny。
 *   - Child Result 消费：从真实 runtime 证明（下一轮 model context 含 reviewer finding），
 *     不再 blackboardConsumed = delegateObserved 假证明。
 *   - Fixture：create 与 cleanup 在同一函数 try/finally（withRealAiFixture），异常路径零残留。
 *   - Budget：attempts/started/succeeded/failed 精确区分，调用前预检。
 *   - Deterministic Integration 未 PASS 前禁止运行真实 DeepSeek（本脚本自动先跑）。
 *
 * Connection 解析优先级（spec §5）：
 ...
 *   5. env fallback（source = env-fallback）
 *
 * 退出码：PASS=0；逻辑 FAIL=1；ENVIRONMENT_FAILURE=2（HTTP auth/quota/network，保留脱敏证据）；
 *         无 Connection=SKIP(0)。
 *
 * 运行：node scripts/real-ai-orchestrator-smoke.js [connectionId]
 */
...
// （electron . 即 main.js）运行时才能解密。plain node / ELECTRON_RUN_AS_NODE /
// 独立 electron 脚本都无法走 §5 的 Store Connection 优先级，因此自动 re-exec 到
...
if (require.main === module && process.type === undefined) {
  ...
  if (electronBin && typeof electronBin === 'string') {
    ...
    if (connectionId) args.push(`--real-ai-connection=${connectionId}`);
...
async function main() {
  const connectionId = process.argv[2] || null;
  log('v2.9.0 Real Runtime Smoke Closure — Real AI Orchestrator Smoke');

  // §7: Deterministic Integration 全绿前禁止烧真实 API
  ...
  // Connection 解析（平台 Store 优先）
  const store = rt.initStandaloneStore();
  const resolved = rt.resolveRealAiConnection(connectionId || process.env.REAL_AI_TEST_CONNECTION_ID || null, { store });
  if (!resolved) {
    console.log('');
    console.log('REAL_AI_ORCHESTRATOR_SMOKE');
  ...
  // §7 生产 Provider 路径：providers.getProvider + createProviderModelAdapter
  const providers = require('../src/providers');
  const { createProviderModelAdapter } = require('../src/agent/runtime/providerModelAdapter');
  const provider = providers.getProvider(resolved.conn);
  ...
  // R8: fixture 的 create 与 cleanup 在同一函数 try/finally
  let exitCode = 1;
  let outcome = null;
  ...
  const leftovers = rt.countFixtureLeftovers();
  ...
  if (!pass && report.parentStatus === 'failed' && report.parentError &&
    ...
    console.log('Note: HTTP auth / quota / network 脱敏证据；修复环境后重跑');
    exitCode = 2;
  } else {
  ...
  console.log(`Fixture leftovers: ${leftovers}`);
  ...
  if (process.env.REAL_AI_RESULT_FILE) {
    try {
      require('fs').writeFileSync(process.env.REAL_AI_RESULT_FILE, JSON.stringify({ status: pass ? 'PASS' : 'FAIL', exitCode, report: { ...report, leftovers } }, null, 2));
    } catch { /* 输出上方 stdout 仍是备选 */ }
  }
  process.exit(exitCode);
}

module.exports = { isEnvironmentFailure, main };
